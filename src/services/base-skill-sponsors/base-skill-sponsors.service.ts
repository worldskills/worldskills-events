...
export class BaseSkillSponsorsService extends WsService<BaseSponsorList> {
  ...
  fetch(p1: WsServiceRequestP1, p2?: WsServiceRequestP2, p3?: WsServiceRequestP3): Observable<BaseSponsorList> {
    ...
    const observable = this.http.get<BaseSponsorList>(
      requestOptions.url ?? `${environment.worldskillsApiEvents}/base_skill/base_sponsors`, {params}
    ).pipe(share());
  ...
  fetchByEntity(entityId: number, p1: WsServiceRequestP1, p2?: WsServiceRequestP2, p3?: WsServiceRequestP3): Observable<BaseSponsorList> {
    ...
    const params = HttpUtil.objectToParams(fetchParams || {});
    const observable = this.http.get<BaseSponsorList>(
      requestOptions.url ?? `${environment.worldskillsApiEvents}/base_skill/base_sponsors?entity=${entityId}`, {params}
    ).pipe(share());
    return this.request(observable, multicastOptions);
  }
}
